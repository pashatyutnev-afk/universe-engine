FILE: UE_V2/03_ENT/10_SPC_ENT/01_CREATIVE_SPC_ENT/00__PIPELINE_CONTRACT__CRV__SPC__ENT.md
...
## [M] PURPOSE
PIPELINE_CONTRACT — навигатор действий для реалма CREATIVE_SPC_ENT.
...
5) Return SPECIALIST_OUTPUT + NEXT prompt
...
- STEP: S2
  GOAL: Creative execution (selection + specialist output)
  INPUTS: [WORK_SET_KEYS, TASK_TOKEN, EXEC_MODE]
  TARGETS: [SPC.CRV.CREATIVE_DIRECTOR, SPC.CRV.VISUAL_STYLE_ARCHITECT, SPC.CRV.WORLD_AESTHETIC_DESIGNER, SPC.CRV.CONCEPT_DESIGNER, SPC.CRV.SYMBOLISM_METAPHOR_DESIGNER, SPC.CRV.MOOD_ATMOSPHERE_CURATOR, SPC.CRV.ARTISTIC_RISK_DESIGNER, SPC.CRV.IDEA_GENERATOR]
  ACTIONS:
    - Resolve selected SPC keys via INDEX_MANIFEST
    - Open only minimal set required by TASK_TOKEN
    - Produce SPECIALIST_OUTPUT (SUMMARY/MAIN/CHECKS/RISKS/NEXT)
